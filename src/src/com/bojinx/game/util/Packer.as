package com.bojinx.game.util
{
	...
	public class Packer extends EventDispatcher
	{
		...
		public function Packer( width:Number, height:Number )
		{
			...
			while ( tmpRects.length > 0 )
			{
				if ( getTimer() - t < timeLimit )
				{
					...
					for ( var i:uint = 0; i < tmpRects.length; ++i )
					{
						...
						if ( score.score1 < bestScore.score1 || ( score.score1 == bestScore.score1 &&
							score.score2 < bestScore.score2 ))
						{
		...
		protected function contactPointScoreNode( x:Number, y:Number, width:Number, height:Number ):Number
		{
			...
			{
				if ( usedRectangles[ i ].x == x + width || usedRectangles[ i ].x + usedRectangles[ i ].
					width == x )
				{
					score += commonIntervalLength( usedRectangles[ i ].y, usedRectangles[ i ].y + usedRectangles[ i ].height, y, y + height );
				}
				
				if ( usedRectangles[ i ].y == y + height || usedRectangles[ i ].y + usedRectangles[ i ].
					height == y )
				{
		...
		protected function findPositionForNewNodeBestAreaFit( width:Number, height:Number, score:Score =
															  null ):Rectangle
		{
			if ( score == null )
			{
			...
			for ( var i:uint = 0; i < freeRectangles.length; ++i )
			{
				...
				if ( freeRectangles[ i ].width >= width && freeRectangles[ i ].height >= height )
				{
					...
					if ( areaFit < score.score1 || ( areaFit == score.score1 && shortSideFit < score.
						score2 ))
					{
		...
		protected function findPositionForNewNodeBestLongSideFit( width:Number, height:Number, score:Score =
																  null ):Rectangle
		{
			if ( score == null )
			{
			...
			for ( var i:uint = 0; i < freeRectangles.length; ++i )
			{
				...
				if ( freeRectangles[ i ].width >= width && freeRectangles[ i ].height >= height )
				{
					...
					if ( longSideFit < score.score2 || ( longSideFit == score.score2 && shortSideFit <
						score.score1 ))
					{
						bestNode.x = freeRectangles[ i ].x;
						bestNode.y = freeRectangles[ i ].y;
		...
		protected function findPositionForNewNodeBestShortSideFit( width:Number, height:Number, score:Score =
																   null ):Rectangle
		{
			if ( score == null )
			{
			...
			for ( var i:uint = 0; i < freeRectangles.length; ++i )
			{
				...
				if ( freeRectangles[ i ].width >= width && freeRectangles[ i ].height >= height )
				{
					...
					if ( shortSideFit < score.score1 || ( shortSideFit == score.score1 && longSideFit <
						score.score2 ))
					{
						bestNode.x = freeRectangles[ i ].x;
						bestNode.y = freeRectangles[ i ].y;
				...
				if ( allowFlip && height != width )
				{
					if ( freeRectangles[ i ].width >= height && freeRectangles[ i ].height >= width )
					{
						...
						if ( flippedShortSideFit < score.score1 || ( flippedShortSideFit == score.score1 &&
							flippedLongSideFit < score.score2 ))
						{
							bestNode.x = freeRectangles[ i ].x;
							bestNode.y = freeRectangles[ i ].y;
							bestNode.width = height;
		...
		protected function findPositionForNewNodeBottomLeft( width:Number, height:Number, score:Score =
															 null ):Rectangle
		{
			if ( score == null )
			{
			...
			for ( var i:uint = 0; i < freeRectangles.length; ++i )
			{
				...
				if ( freeRectangles[ i ].width >= width && freeRectangles[ i ].height >= height )
				{
					...
					if ( topSideY < score.score1 || ( topSideY == score.score1 && freeRectangles[ i ].
						x < score.score2 ))
					{
						bestNode.x = freeRectangles[ i ].x;
						bestNode.y = freeRectangles[ i ].y;
		...
		protected function findPositionForNewNodeContactPoint( width:Number, height:Number, score:Score =
															   null ):Rectangle
		{
			if ( score == null )
			{
		...
		protected function splitFreeNode( freeNode:Rectangle, usedNode:Rectangle ):Boolean
		{
			...
			if ( usedNode.x < freeNode.x + freeNode.width && usedNode.x + usedNode.width > freeNode.
				x )
			{
			...
			if ( usedNode.y < freeNode.y + freeNode.height && usedNode.y + usedNode.height > freeNode.
				y )
			{
